...
const GheeJarIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.4} strokeLinecap="round" strokeLinejoin="round" className="w-[42px] h-[42px]">
...
export default function ExploreCategories() {
  ...
  const scrollByCard = (dir) => {
    ...
    const step = card ? card.offsetWidth + 24 : 224;
        ...
        <div className="relative">
          ...
          >
            {CATEGORIES.map(({ name, Icon, path }) => (
              <div
                key={name}
                className="group shrink-0 w-[200px] h-[196px] lg:w-[calc((100%-6rem)/5)] lg:h-[210px] bg-white rounded-xl p-5 flex flex-col items-center justify-center text-center hover:-translate-y-1 hover:shadow-[0_12px_28px_rgba(0,0,0,0.1)] transition-all duration-300"
                style={{ boxShadow: "0 1px 6px rgba(0,0,0,0.06)" }}
              >
                <span className="text-[#a02c2c]">
                  <Icon />
                </span>
                <h3 className="font-body text-[#333] text-[15px] font-medium leading-snug mt-3">
                  {name}
                </h3>
                <div className="w-3/4 border-t border-dashed border-gray-200 my-2.5" />
                <a
                  href={path}
